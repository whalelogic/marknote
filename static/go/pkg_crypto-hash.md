# Cryptography and Hashing in Go

1. The `crypto` package in Go serves as a foundational umbrella for various cryptographic subpackages. It defines common interfaces and constants used by specific implementations like `crypto/aes` for symmetric encryption, `crypto/rsa` or `crypto/ed25519` for asymmetric operations, and `crypto/sha256` for hashing.
